...
class EventSeeder extends BaseSeeder
{
    public function run(): void
    {
        DB::table('events')->whereIn('nama_event', [
            'Job Fair Tech Career 2026',
            'Workshop Membuat CV Profesional',
            'Tokopedia DevCamp 2026',
            'Blibli Code Blitz Webinar',
        ])->delete();

        $perusahaanId1 = DB::table('profile_perusahaan')
            ->where('email', 'hrd@techmuda.com')
            ->value('id');

        $perusahaanId2 = DB::table('profile_perusahaan')
        ...
        if ($perusahaanId1) {
            $this->upsertAndGetId('events', ['nama_event' => 'Job Fair Tech Career 2026'], [
        ...
        if ($perusahaanId2) {
            $this->upsertAndGetId('events', ['nama_event' => 'Workshop Membuat CV Profesional'], [
