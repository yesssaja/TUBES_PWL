...
<div class="max-w-5xl mx-auto">
    ...
    <form action="{{ route('perusahaan.course.update', $course->id) }}"
          method="POST"
        ...
        <div class="mb-5">
            ...
            <textarea name="benefit"
                      rows="4"
                      class="w-full border border-gray-200 rounded-2xl px-4 py-3 focus:outline-none focus:ring-4 focus:ring-red-100 focus:border-red-400 resize-none">{{ old('benefit', $course->benefit) }}</textarea>
        </div>
        ...
        <div class="mb-5">
            ...
            <select name="payment_required"
                    class="w-full border border-gray-200 rounded-2xl px-4 py-3 focus:outline-none focus:ring-4 focus:ring-red-100 focus:border-red-400">
                <option value="1" {{ old('payment_required', $course->payment_required) == 1 ? 'selected' : '' }}>Ya</option>
                <option value="0" {{ old('payment_required', $course->payment_required) == 0 ? 'selected' : '' }}>Tidak</option>
            </select>
        </div>
        ...
        <div class="mb-5">
            ...
            <textarea name="payment_note"
                      rows="4"
                      class="w-full border border-gray-200 rounded-2xl px-4 py-3 focus:outline-none focus:ring-4 focus:ring-red-100 focus:border-red-400 resize-none">{{ old('payment_note', $course->payment_note) }}</textarea>
        </div>

        @php
            $link = $course->links->first() ?? null;
        @endphp
