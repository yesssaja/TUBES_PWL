...
@section('content')
    ...
    <div class="min-h-screen bg-gradient-to-br from-[#FFF7E8] via-white to-red-50 px-4 py-10">
        ...
        <div class="max-w-5xl mx-auto">
            ...
            {{-- HEADER --}}
            <div
                class="bg-gradient-to-br from-red-600 via-orange-500 to-yellow-400 rounded-[32px] p-7 md:p-9 text-white shadow-2xl mb-8">
                <div class="flex flex-col md:flex-row md:items-center gap-5">

                    <div
                        class="w-24 h-24 rounded-3xl bg-white text-red-600 flex items-center justify-center text-5xl font-black shadow-xl overflow-hidden">
                        @if ($profile?->foto_diri)
                            <img src="{{ asset('storage/' . $profile->foto_diri) }}" class="w-full h-full object-cover"
                                alt="Foto Diri">
                        @else
            ...
            @if (session('success'))
                <div
                    class="bg-green-50 border border-green-200 text-green-700 px-5 py-4 rounded-2xl mb-6 font-semibold shadow-sm">
                    {{ session('success') }}
                </div>
            @endif
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                            ...
                            <div>
                                ...
                                <input type="text" value="{{ $profile?->nik ?? '' }}" readonly
                                    class="block w-full px-5 py-4 rounded-2xl border border-gray-200 bg-gray-100 text-sm text-gray-500 cursor-not-allowed">
                            </div>
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                            ...
                            <div class="bg-gray-50 rounded-3xl border border-gray-100 p-5">
                                ...
                                @if ($profile?->foto_diri)
                                    <img src="{{ asset('storage/' . $profile->foto_diri) }}" alt="Foto Diri"
                                        class="w-24 h-24 rounded-2xl object-cover mb-3 border">
                                @else
                                    <div
                                        class="w-24 h-24 rounded-2xl bg-red-100 text-red-600 flex items-center justify-center text-4xl font-black mb-3">
                                        {{ strtoupper(substr($user->name, 0, 1)) }}
                                    </div>
                                @endif
                            ...
                            <div class="bg-gray-50 rounded-3xl border border-gray-100 p-5">
                                ...
                                @if ($profile?->foto_ktp)
                                    <img src="{{ asset('storage/' . $profile->foto_ktp) }}" alt="Foto KTP"
                                        class="w-full h-24 rounded-2xl object-cover mb-3 border">
                                @else
                                    <div
                                        class="w-full h-24 rounded-2xl bg-gray-100 text-gray-400 flex items-center justify-center mb-3 font-bold">
                                        Belum ada
                                    </div>
                                @endif

                                <p class="text-xs text-gray-500 font-semibold">
                                    Dokumen ini tidak dapat diubah.
                                </p>
                            </div>
                            ...
                            <div class="bg-gray-50 rounded-3xl border border-gray-100 p-5">
                                ...
                                @if ($profile?->foto_ijazah)
                                    <img src="{{ asset('storage/' . $profile->foto_ijazah) }}" alt="Foto Ijazah"
                                        class="w-full h-24 rounded-2xl object-cover mb-3 border">
                                @else
                                    <div
                                        class="w-full h-24 rounded-2xl bg-gray-100 text-gray-400 flex items-center justify-center mb-3 font-bold">
                        ...
                        <p class="text-xs text-gray-400 mt-3">
                            HANYA FOTO DIRI YANG DAPAT DIPERBARUI
                        </p>
                    </div>
